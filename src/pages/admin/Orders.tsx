...
import { Search, ChevronRight, LayoutList, Columns3, Clock, Truck, MapPin } from "lucide-react";
...
function KanbanCard({ order, onClick }: { order: Order; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="p-3 bg-card rounded-lg border border-border/50 cursor-pointer hover:shadow-md hover:border-border transition-all group"
    >
        ...
        <div className="overflow-x-auto pb-4 -mx-4 md:-mx-6 px-4 md:px-6">
          <div className="flex gap-3 min-w-[900px]">
            {KANBAN_COLUMNS.map((col) => {
              ...
              return (
                <div key={col.status} className={`flex-1 min-w-[220px] bg-muted/30 rounded-xl border-t-[3px] ${col.color}`}>
